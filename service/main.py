...
if IS_ANDROID:
    ...
    # NotificationCompat lives in androidx.core.  Ensure the following line is
    # present in your buildozer.spec (or the equivalent Gradle block in codemagic):
    #   android.add_gradle_maven_dependencies = androidx.core:core:1.12.0
...
def main():
    ...
    # mp_result_code is passed as a JSON field in python_service_argument.
    # mp_data (the MediaProjection Intent Parcelable) is a separate Intent
    # extra added by overlay._start_service via si.putExtra("mp_data", ...).
    mp_rc   = int(args.get("mp_result_code", -1))
    mp_data = None
    if IS_ANDROID and mp_rc != -1:
        try:
            # p4a sets PythonService.mService before running Python; the start
            # Intent (containing our "mp_data" Parcelable) is available on it.
            mp_data = PythonService.mService.getIntent().getParcelableExtra("mp_data")
        except Exception as _exc:
            Logger.warning(
                f"SoccerStarsService: could not retrieve mp_data Intent "
                f"({_exc}). Screen capture disabled for this session."
            )
            mp_rc = -1
