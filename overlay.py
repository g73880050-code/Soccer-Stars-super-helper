...
class FloatingOverlayManager:
    ...
    def _start_service(self):
        ...
        service_arg = {
            ...
            "mp_result_code": self._mp_token[0] if self._mp_token else -1,
        }
        si.putExtra("python_service_argument", json.dumps(service_arg))
        # Pass the MediaProjection data Intent as a Parcelable extra so the
        # service can call mpm.getMediaProjection(resultCode, dataIntent).
        if self._mp_token is not None:
            si.putExtra("mp_data", self._mp_token[1])
